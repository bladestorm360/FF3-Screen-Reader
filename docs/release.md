...
```
powershell.exe -Command "& 'D:\Games\Dev\Unity\FFPR\ff3\ff3-screen-reader\build_and_deploy.bat'"
```

Confirms compile succeeds and produces `bin\Debug\net6.0\FFIII_ScreenReader.dll`. If the build fails, stop.
...
| File | Source |
|---|---|
| `FFIII_ScreenReader.dll` | `bin\Debug\net6.0\FFIII_ScreenReader.dll` (the fresh build) |
...
```
& "C:\Program Files\7-Zip\7z.exe" a -tzip "Releases\FFIII-Screen-ReaderV<version>.zip" ".\Releases\V<version>\*"
```

Zip naming: `FFIII-Screen-ReaderV<version>.zip`, placed in `Releases\` (sibling of the version directory, not inside it). The zip's root contains the four files directly — no nested `V<version>\` folder, so end users can extract and follow ReadMe placement instructions without re-routing paths.

### 4. Tag and push

```
git tag V<version>
git push ff3-screen-reader master
git push ff3-screen-reader V<version>
```

Remote name is `ff3-screen-reader` (not `origin`). Default branch is `master`. Verify with `git remote -v` if unsure. Push `master` first so the branch points at the released commit (otherwise the tagged commit only reaches the remote via the tag ref, leaving remote `master` behind HEAD). Then push the tag.
...
**If `V<previous>` has no tag locally** (FF3 has historically shipped zips ahead of the latest pushed tag), use the commit closest in time to the prior `Releases\V<previous>\` directory's mtime as the baseline for the changelog diff.
...
| Per-frame tile-crossing footsteps silent in vehicles | Footsteps no longer pip while traveling in vehicles |
...
```
gh release create V<version> "Releases\FFIII-Screen-ReaderV<version>.zip" --title "V<version>" --notes-file "release-notes-V<version>.txt"
```
...
```
https://github.com/bladestorm360/FF3-Screen-Reader/releases/tag/V<version>
```
...
- Does **not** bump `<Version>` in `FFIII_ScreenReader.csproj`. The csproj version is not currently used as the release source-of-truth; do not change it as part of release prep unless the user asks.
