...
| Config Menu | Validate via `activeInHierarchy`. `ConfigActualDetailsControllerBase.SelectCommand` for nav (replaced the `SetFocus` attribute patch), `SwitchArrowSelectTypeProcess` for arrow values, `Slider.onValueChanged` listeners for slider values; after the config bestiary, the focused row is re-read from the library-return fade-in callback `FieldMap.<InitMenu>b__90_0` |
...
- config value changes (KeyInput `SwitchArrowSelectTypeProcess` 0x309430, Touch `SwitchArrowTypeProcess` 0x8908D0 / `SwitchSliderTypeProcess` 0x890F20; the KeyInput `SwitchSliderTypeProcess` hook was later replaced by `Slider.onValueChanged`, see "Follow-up" below), equipment `SelectContent` (0x56F260, 0x573C80), item list / item target `SelectContent` (0x7BC180; 0x982E80 shares its body only with the same class's `SetCursor(IEnumerable, Cursor)`), wall-bump `FieldPlayerKeyController.OnTouchPadCallback` 0x71BE50.
...
**Follow-up (after 6d3b6df): the last two polls converted.**
- Config slider: the KeyInput `SwitchSliderTypeProcess` hook (0x30A740) is removed. The game calls it every frame from the tail of `UpdateController` with a null `Key` while a slider row is focused, and re-asserts the value there (`SetSliderValue` 0x6231B0 → `Slider.set_value`, `ConfigClient.SetVolume` / `SetBrightness`), so no game method runs only on a change. Only the left/right path (the input lambda `<UpdateController>b__0`, 0x633F80) moves the value, and Unity's `Slider.onValueChanged` fires only when the value really changes. `ConfigSliderValueListener` adds one listener per slider when its row is first read (`AnnounceSelectedConfigCommand`), and reads the focused row's slider one frame after the event (percentage over the slider's min/max, `interrupt: true`; several events in a frame read once; same slider + same value is not repeated). At either end of the range the value does not move, so nothing is spoken. Reset when the config menu closes.
- Config-bestiary return: the `WaitForSeconds(0.1)` wait of up to 10 s is removed. `MenuExtraLibraryUi.GotoMenu` passes `FieldMap.CreateReturnMonsterLibraryArguemtns` (0x34C0F0, its only caller), so `FieldMap.InitMenu` (0x34D800, the Menu-state entry) takes its library-return branch: `FadeManager.FadeIn`, then a delegate stored in the FadeManager's completion callback (0x58) whose method is `FieldMap.<InitMenu>b__90_0` (0x352170, unique; the only `<InitMenu>` lambda; interop name `_InitMenu_b__90_0`). The lambda switches FieldMap's menu state machine (0xD0) to state 1, i.e. the menu is back on screen. The bestiary exit (`GameStatePatches`) now only arms the re-read; the lambda's postfix consumes the arm and reads the focused row, retrying at most 10 frames until it is active (last details controller first, then `ConfigController.detailsController` 0x48), and gives up if `SelectCommand` read a row since the arm. `ConfigController.SetActive(false)` clears the arm.
...
- **Parity wording (2026-09-25):** the beacon toggle now says "Beacon navigation on/off" (new key `Beacon navigation {0}`) and its mod-menu row is "Beacon Navigation" (new key), as in FF1, FF2 and FF4. The old `Audio beacons {0}` / `Audio Beacons` keys are unused.
